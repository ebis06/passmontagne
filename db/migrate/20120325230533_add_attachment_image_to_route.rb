class AddAttachmentImageToRoute < ActiveRecord::Migration
  def self.up
    add_column :routes, :image_file_name, :string
    add_column :routes, :image_content_type, :string
    add_column :routes, :image_file_size, :integer
    add_column :routes, :image_updated_at, :datetime
  end
  ...
  def self.down
    ...
    remove_column :routes, :image_updated_at
  end
end
